...
internal sealed partial class PokemonServerListener : BackgroundService, IPokemonServerListener
{
    ...
    private async Task RunServerPortCheckingTask(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                await using var streamWriter = new StreamWriter(tcpClient.GetStream());

                await streamWriter.WriteLineAsync(new ServerDataRequestPacket().ToRawPacket().AsMemory(), globalCancellationTokenSource.Token).ConfigureAwait(false);
                await streamWriter.FlushAsync(globalCancellationTokenSource.Token).ConfigureAwait(false);
                ...
                {
                    PrintServerInfo((ServerInfoDataPacket) packet);
    ...
    [LoggerMessage(Level = LogLevel.Debug, Message = "[Server] Received {serverInfoDataPacket}")]
    private partial void PrintServerInfo(ServerInfoDataPacket serverInfoDataPacket);
