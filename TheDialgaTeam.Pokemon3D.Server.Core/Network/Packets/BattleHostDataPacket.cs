...
public sealed record BattleHostDataPacket(Origin Origin, Origin BattlePartner, string Data) : IPacket
{
    ...
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleHostData, Origin, new[] { Data });
    }

    public IRawPacket ToClientRawPacket()
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleHostData, Origin, new[] { BattlePartner.ToRawPacketString(), Data });
    }
}