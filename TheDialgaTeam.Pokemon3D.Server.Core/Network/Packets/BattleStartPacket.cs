...
public record BattleStartPacket(Origin Origin, Origin BattlePartner) : IPacket
{
    ...
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleStart, Origin, Array.Empty<string>());
    }

    public IRawPacket ToClientRawPacket()
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleStart, Origin, new[] { BattlePartner.ToRawPacketString() });
    }
}