...
public class MicrosoftExtensionsOptionsServerOptionsProvider(IOptionsMonitor<ServerOptions> optionsMonitor) : IServerOptionsProvider
{
    ...
    {
        if (optionsMonitor.CurrentValue.GameModeOverrides.TryGetValue(gameMode, out var overrideOptions))
        {
            overrideOptions.SetDefaults(optionsMonitor.CurrentValue);
            return overrideOptions;
        }

        var defaultOverrideOptions = new GameModeOverrideOptions();
        defaultOverrideOptions.SetDefaults(optionsMonitor.CurrentValue);
        return defaultOverrideOptions;
    }

    public IDisposable? OnChange(Action<ServerOptions, string?> listener)
    {
        return optionsMonitor.OnChange(listener);
    }
}