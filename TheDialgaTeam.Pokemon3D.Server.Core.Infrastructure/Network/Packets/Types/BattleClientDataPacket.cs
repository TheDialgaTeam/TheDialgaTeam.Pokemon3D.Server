...
public sealed record BattleClientDataPacket(Origin Origin, Origin BattlePartner, string Data) : IPacket
{
    ...
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleClientData, Origin, [Data]);
    }

    public IRawPacket ToClientResponseRawPacket()
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleClientData, Origin, [BattlePartner.ToRawString(), Data]);
    }
}