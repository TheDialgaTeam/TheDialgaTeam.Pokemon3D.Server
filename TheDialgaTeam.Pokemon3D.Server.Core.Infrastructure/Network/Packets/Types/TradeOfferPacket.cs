...
public sealed record TradeOfferPacket(Origin Origin, Origin TradePartner, string PokemonData) : IPacket
{
    ...
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.TradeOffer, Origin, [PokemonData]);
    }

    public IRawPacket ToClientResponseRawPacket()
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.TradeOffer, Origin, [TradePartner.ToRawString(), PokemonData]);
    }
}