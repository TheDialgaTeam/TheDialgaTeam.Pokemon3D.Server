...
public sealed record BattleQuitPacket(Origin Origin, Origin BattlePartner) : IPacket
{
    ...
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleQuit, Origin, []);
    }

    public IRawPacket ToClientResponseRawPacket()
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleQuit, Origin, [BattlePartner.ToRawString()]);
    }
}