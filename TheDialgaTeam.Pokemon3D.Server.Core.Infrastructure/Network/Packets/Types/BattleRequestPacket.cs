...
public sealed record BattleRequestPacket(Origin Origin, Origin BattlePartner) : IPacket
{
    ...
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleRequest, Origin, []);
    }

    public IRawPacket ToClientResponseRawPacket()
    {
        return new RawPacket(RawPacket.ProtocolVersion, PacketType.BattleRequest, Origin, [BattlePartner.ToRawString()]);
    }
}